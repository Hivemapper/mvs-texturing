...
void textureMesh(const TextureSettings &texture_settings,
  ...
  //    if (atlas_size == 0) { // only used in commented out
  //    generate_capped_texture_atlas method below
  //        atlas_size = 4096;
  //    }
  ...
  settings.geometric_visibility_test =
      texture_settings.do_geometric_visibility_test;
  ...
  if (labeling_file.empty()) {
    ...
    std::vector<std::size_t> labeling =
        vector_from_file<std::size_t>(labeling_file);
  ...
  tex::TexturePatches texture_object_class_patches;
  {
    ...
    tex::generate_texture_patches(graph, mesh, mesh_info, &texture_views,
                                  settings, &vertex_projection_infos,
                                  &texture_patches);
...
#pragma omp parallel for schedule(dynamic)
        for (std::size_t i = 0; i < texture_object_class_patches.size(); ++i) {
          ...
          std::vector<math::Vec3f> patch_adjust_values(
              ...
              math::Vec3f(0.0f));
          texture_object_class_patch->adjust_colors(patch_adjust_values,
      ...
      if (settings.local_seam_leveling) {
        if (do_texture_atlas) {
          ...
          tex::local_seam_leveling_n(graph, mesh, vertex_projection_infos,
                                     &texture_patches, num_texture_channels,
      ...
      if (segmentation_classes != nullptr) {
        ...
        for (std::size_t i = 0; i < vertex_projection_infos.size(); ++i) {
          std::vector<tex::VertexProjectionInfo> const &projection_infos =
              vertex_projection_infos[i];
          std::vector<float> texture_channels(num_texture_channels);
          int number_projections = 0;
          for (tex::VertexProjectionInfo const &projection_info :
               projection_infos) {
            ...
            std::transform(texture_channels.begin(), texture_channels.end(),
                           pixel_channels.begin(), texture_channels.begin(),
                           std::plus<float>());
          }
...
#pragma omp parallel for schedule(dynamic)
    for (std::size_t vi = 0; vi < sub_vert_masks.size(); ++vi) {
      ...
      size_t patch_ct = 0;
      for (std::size_t i = 0; i < texture_patches.size(); ++i) {
        TexturePatch::Ptr new_patch =
            TexturePatch::create(texture_patches[i], face_indices);
                ...
                << texture_patches.size() << " patches." << std::endl;
      {
        ...
        // capped method is desireable, but resizes currently have some issues
        // that make it worse.
        ...
        //                                    mesh->get_vertices(),
        //                                    mesh->get_faces());
        tex::generate_texture_atlases(&sub_texture_patches, settings,
                                      &sub_texture_atlases,
                                      mesh->get_vertices(), mesh->get_faces());
      }
      ...
      if (num_texture_channels > num_colors) {
        ...
        // color mapping (nice for testing) or remove this
        {
          ...
          tex::generate_texture_atlases(
              &sub_texture_object_class_patches, settings,
              &sub_texture_object_class_atlases, mesh->get_vertices(),
              mesh->get_faces());
        }
        ...
        /* Create and write out obj model for object classes. */
        {
          ...
          tex::Model sub_model{};
          tex::build_model(sub_mesh, sub_texture_object_class_atlases,
                           &sub_model);
