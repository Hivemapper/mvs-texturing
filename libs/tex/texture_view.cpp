...
void TextureView::get_face_info(
    math::Vec3f const& v1,
    math::Vec3f const& v2,
    math::Vec3f const& v3,
...
bool TextureView::valid_pixel(math::Vec2f pixel) const {
  ...
  bool valid =
      (x >= 0.0f && x < static_cast<float>(width - 1) && y >= 0.0f
...
void TextureView::export_triangle(
    math::Vec3f v1,
    math::Vec3f v2,
    math::Vec3f v3,
  ...
  mve::image::save_jpg_file(
      mve::image::crop(
          image, width, height, left, top, *math::Vec3uc(255, 0, 255)),
      filename,
      80);
}
...
void TextureView::export_validity_mask(std::string const& filename) const {
  ...
  mve::image::save_jpg_file(img, filename, 80);
}
