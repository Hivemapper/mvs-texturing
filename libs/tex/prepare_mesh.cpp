...
std::size_t remove_redundant_faces(
  ...
  for (std::size_t i = 0; i < faces.size(); i += 3) {
    ...
    for (std::size_t j = 0; !redundant && j < 3; ++j) {
      mve::MeshInfo::AdjacentFaces const& adj_faces =
          mesh_info[faces[i + j]].faces;
      for (std::size_t k = 0; !redundant && k < adj_faces.size(); ++k) {
        ...
        if (face_id < adj_face_id) {
          bool identical = true;
          /* Faces are considered identical if they consist of the same
           * vertices. */
    ...
    if (redundant) {
      ...
      new_faces.insert(
          new_faces.end(), faces.cbegin() + i, faces.cbegin() + i + 3);
    }
  }
...
void prepare_mesh(mve::MeshInfo* mesh_info, mve::TriangleMesh::Ptr mesh) {
  ...
  if (num_redundant > 0) {
    std::cout << "\tRemoved " << num_redundant << " redundant faces."
              << std::endl;
  }
