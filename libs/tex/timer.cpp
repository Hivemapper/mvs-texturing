...
void Timer::measure(std::string const& eventname) {
  ...
  if (events.empty()) {
    Event event = {
        eventname, abs_clocks, abs_milliseconds, abs_clocks, abs_milliseconds};
    events.push_back(event);
  } else {
    Event last_event = events.back();
    std::size_t rel_clocks = abs_clocks - last_event.abs_clocks;
    std::size_t rel_milliseconds =
        abs_milliseconds - last_event.abs_milliseconds;

    Event event = {
        eventname, abs_clocks, abs_milliseconds, rel_clocks, rel_milliseconds};
    events.push_back(event);
  }
}
...
void Timer::write_to_file(std::string const& filename) const {
  ...
  out << "Event, Absolute clocks, Absolute milliseconds, Relative clocks, "
         "Relative milliseconds"
      << std::endl;
  for (std::size_t i = 0; i < events.size(); ++i) {
    Event event = events[i];
    out << event.name << ", " << event.abs_clocks << ", "
        << event.abs_milliseconds << ", " << event.rel_clocks << ", "
