...
math::Vec3f calculate_difference(
    ...
    std::vector<MeshEdge> const& seam_edges,
    int label1,
    int label2) {
  assert(label1 != 0 && label2 != 0 && label1 < label2);
  ...
  for (MeshEdge const& seam_edge : seam_edges) {
    ...
    find_mesh_edge_projections(
        vertex_projection_infos, seam_edge, &edge_projection_infos);
...
void global_seam_leveling(
      ...
      for (std::size_t k = 0; k < adj_verts.size(); ++k) {
        ...
        for (std::size_t l = 0; l < labels[adj_vertex].size(); ++l) {
          ...
          if (i < adj_vertex && label == adj_vertex_label) {
            Eigen::Triplet<float, int> t1(
                Gamma_row, vertlabel2row[i][label], lambda);
            Eigen::Triplet<float, int> t2(
                Gamma_row,
                vertlabel2row[adj_vertex][adj_vertex_label],
                -lambda);
  ...
  assert(
      Gamma_rows < static_cast<std::size_t>(std::numeric_limits<int>::max()));
  ...
  for (std::size_t i = 0; i < texture_patches->size(); ++i) {
    ...
    //  Only adjust texture_patches originating from input images.
    if (label == 0) {
