...
bool RectangularBin::insert(Rect<int>* rect) {
  ...
  if (best_rect_it != rects.end()) {
    ...
    /* Decide split axis. */
    Rect<int> hsplit_top(
        best_rect.min_x, rect->max_y, best_rect.max_x, best_rect.max_y);
    Rect<int> hsplit_bottom(
        rect->max_x, best_rect.min_y, best_rect.max_x, rect->max_y);
    Rect<int> vsplit_left(
        best_rect.min_x, rect->max_y, rect->max_x, best_rect.max_y);
    Rect<int> vsplit_right(
        rect->max_x, best_rect.min_y, best_rect.max_x, best_rect.max_y);
    ...
    if (hsplit_top.size() != 0 && hsplit_bottom.size() != 0)
      hsplit_ratio =
          static_cast<float>(hsplit_top.size()) / hsplit_bottom.size();
    if (vsplit_left.size() != 0 && vsplit_right.size() != 0)
      vsplit_ratio =
          static_cast<float>(vsplit_left.size()) / vsplit_right.size();

    if (std::abs(1.0f - hsplit_ratio) < std::abs(1.0f - vsplit_ratio)) {
      if (vsplit_left.size() != 0)
        rects.push_back(vsplit_left);
      if (vsplit_right.size() != 0)
        rects.push_back(vsplit_right);
    } else {
      if (hsplit_top.size() != 0)
        rects.push_back(hsplit_top);
      if (hsplit_bottom.size() != 0)
        rects.push_back(hsplit_bottom);
    }
