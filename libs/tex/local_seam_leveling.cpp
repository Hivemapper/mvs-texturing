...
std::vector<float> mean_color_of_edge_point_n(
    std::vector<EdgeProjectionInfo> const& edge_projection_infos,
    std::vector<TexturePatch::Ptr> const& texture_patches,
    float t,
...
void local_seam_leveling(
  ...
  std::vector<std::vector<EdgeProjectionInfo>> edge_projection_infos {};
  {
    ...
    for (std::size_t i = 0; i < seam_edges.size(); ++i) {
      ...
      find_mesh_edge_projections(
          vertex_projection_infos, seam_edge, &edge_projection_infos[i]);
    }
  }
  ...
  for (std::size_t i = 0; i < edge_projection_infos.size(); ++i) {
    ...
    for (std::size_t j = 0; j < edge_color.size(); ++j) {
      ...
      edge_color[j] = mean_color_of_edge_point(
          edge_projection_infos[i], *texture_patches, t);
    }
    ...
    for (EdgeProjectionInfo const& edge_projection_info :
      ...
      lines[edge_projection_info.texture_patch_id].emplace_back(
          std::move(line));
    }
  }
  ...
  for (std::size_t i = 0; i < texture_patches->size(); ++i) {
    ...
    /* Only alter a small strip of texture patches originating from input
     * images. */
    if (texture_patch->get_label() != 0) {
...
void local_seam_leveling_n(
  ...
  for (std::size_t i = 0; i < edge_projection_infos.size(); ++i) {
    ...
    for (std::size_t j = 0; j < edge_color.size(); ++j) {
      float t = static_cast<float>(j) / (edge_color.size() - 1);
      edge_color[j] = mean_color_of_edge_point_n(
          edge_projection_infos[i], *texture_patches, t, num_texture_channels);
    }
  ...
  for (std::size_t i = 0; i < vertex_colors.size(); ++i) {
    ...
    for (VertexProjectionInfo const& projection_info : projection_infos) {
      ...
      if (texture_patch->get_label() == 0)
        continue;
      num_textures++;
      auto color = texture_patch->get_pixel_value_n(
