...
void build_model(
  ...
  mve::TriangleMesh::NormalList const& mesh_normals =
  ...
  for (TextureAtlas::Ptr texture_atlas : texture_atlases) {
    ...
    TextureAtlas::Texcoords const& atlas_texcoords =
        texture_atlas->get_texcoords();
    TextureAtlas::TexcoordIds const& atlas_texcoord_ids =
        texture_atlas->get_texcoord_ids();

    std::size_t texcoord_id_offset = texcoords.size();

    texcoords.insert(
        texcoords.end(), atlas_texcoords.begin(), atlas_texcoords.end());

    for (std::size_t i = 0; i < atlas_faces.size(); ++i) {
      std::size_t mesh_face_pos = atlas_faces[i] * 3;

      std::size_t vertex_ids[] = {mesh_faces[mesh_face_pos],
