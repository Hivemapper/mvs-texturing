...
template <typename T>
inline const std::string choice_string(T i) {
  return choice_strings<T>()[static_cast<std::size_t>(i)];
}

template <typename T>
inline const std::string choices() {
...
template <typename T>
inline T parse_choice(std::string s) {
  const std::vector<std::string> strings = choice_strings<T>();
  const std::size_t n = strings.size();
  for (std::size_t i = 0; i < n; ++i) {
    if (s == strings[i]) {
...
enum DataTerm { DATA_TERM_AREA = 0, DATA_TERM_GMI = 1 };

/** Enum representing a smoothness term. */
enum SmoothnessTerm { SMOOTHNESS_TERM_POTTS = 0 };
...
enum ToneMapping { TONE_MAPPING_NONE = 0, TONE_MAPPING_GAMMA = 1 };
...
template <>
inline const std::vector<std::string> choice_strings<tex::DataTerm>() {
  return {"area", "gmi"};
}

template <>
inline const std::vector<std::string> choice_strings<tex::SmoothnessTerm>() {
  return {"potts"};
}

template <>
inline const std::vector<std::string> choice_strings<tex::OutlierRemoval>() {
  return {"none", "gauss_damping", "gauss_clamping"};
}

template <>
inline const std::vector<std::string> choice_strings<tex::ToneMapping>() {
  return {"none", "gamma"};
}
