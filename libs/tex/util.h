 ...
 * integral is 1.
 *
 * @param X is the vector for which the function is to be evaluated. Must have
 * size 1xN.
 * @param mu is the mean around which the distribution is centered. Must have
 * size 1xN.
 * @param covariance_inv is the INVERSE of the covariance matrix. Must have size
 * NxN.
 * @return \f$\exp(-\frac{1}{2} (X-\mbox{mu})^T \cdot \mbox{covariance\_inv}
 * \cdot (X-\mbox{mu}))\f$
 */
template <typename T, int N>
T const multi_gauss_unnormalized(
    Eigen::Matrix<T, 1, N> const& X,
    Eigen::Matrix<T, 1, N> const& mu,
    Eigen::Matrix<T, N, N> const& covariance_inv) {
  Eigen::Matrix<T, 1, N> mean_removed = X - mu;
  return std::exp(
      T(-0.5) * mean_removed * covariance_inv * mean_removed.adjoint());
}
...
void vector_to_file(std::string const& filename, std::vector<T> const& vector) {
  ...
  out.write(
      reinterpret_cast<const char*>(&vector[0]), vector.size() * sizeof(T));
  out.close();
}
 ...
 * @throws util::FileException
 */
inline void write_string_to_file(
    std::string const& filename,
